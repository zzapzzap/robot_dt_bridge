...
어디서 막히는지 바로 보여 준다.
    ...
    python3 tools/plc_probe.py --write D2000 0 1 0        # run=0 hold=1 stop=0
"""
...
def load_defaults(profile: str):
    try:
        import yaml
        with (ROOT / "config" / "plc.yaml").open(encoding="utf-8") as f:
            cfg = yaml.safe_load(f)
        return cfg["profiles"][profile]["connection"], cfg
    except Exception:
        return {}, {}
...
def main() -> int:
    ...
    if a.host:  conn["host"] = a.host
    if a.port:  conn["port"] = a.port
    if a.frame: conn["frame"] = a.frame
    if not conn.get("host"):
        print("host 를 지정하세요 (--host 또는 config/plc.yaml)"); return 2
    ...
    try:
        ...
        if not a.dump:
            ...
            print("  6축 원시값 → 각도 (scale 0.001 가정)")
            for n, r in zip(names, raw):
                print(f"    {n:<4s} raw {r:>10d}   →  {r * 0.001:8.3f}°")
        ...
        if a.watch:
            ...
            while True:
                ...
                line = "  ".join(f"{r * 0.001:8.2f}" for r in raw)
